...
Start MongoDB, then run auth-service on `8001`, branch-service on `3002`, and gateway on `8000`. Set the same `JWT_ACCESS_SECRET` in auth-service and gateway, and the same `GATEWAY_SHARED_SECRET` in gateway and branch-service.
...
curl http://localhost:8000/api/v1/auth/me -H "Authorization: Bearer <accessToken>"
curl "http://localhost:8000/api/v1/branches?page=1&limit=10" -H "Authorization: Bearer <accessToken>"
```
